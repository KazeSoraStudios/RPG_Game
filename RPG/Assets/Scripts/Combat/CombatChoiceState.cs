...
public class CombatChoiceState : MonoBehaviour, IGameState
{
    public class Config
    {
    ...
    public void Init(Config config)
    {
        if (config == null)
        {
            LogManager.LogError("Combat Choice State passed null config.");
            return;
        }
        time = 0.0f;
        ...
        marker.transform.position = character.Entity.GetSelectPosition();
    }
    ...
    ////public void CreateActionTargeter(def, browseState, combatEvent)
    ...
    public bool Execute(float deltaTime)
    {
        ...
        var bounce = new Vector3(position.x, Mathf.Sin(time * Constants.SELECT_MARKER_BOUNCE_SPEED), 0.0f); ;
        marker.transform.position = bounce;
        return true;
    }
    ...
    function CombatChoiceState:TakeAction(id, targets)
        ...
        if id == "attack" then
            ...
            local tp = queue:SpeedToTimePoints(1)
            queue:Add(attack, tp)
