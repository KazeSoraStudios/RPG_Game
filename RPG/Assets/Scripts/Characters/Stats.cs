...
public class Stats : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        var slots = (Actor.EquipSlot[])Enum.GetValues(typeof(Actor.EquipSlot));
    ...
    public void AddModifier(Actor.EquipSlot slot, StatModifier modifier)
    {
    ...
    public void RemoveModifier(Actor.EquipSlot slot)
    {
        if ((int)slot > modifiers.Length)
        {
    ...
    public int GetStatDiffForNewItem(Stat stat, Actor.EquipSlot slot, StatModifier newModifier)
    {
        if (!stats.ContainsKey(stat))
            return 0;
